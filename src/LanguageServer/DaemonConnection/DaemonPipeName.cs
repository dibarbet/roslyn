...
/// server daemon. Mirrors the algorithm used by the compiler server (BuildServerConnection) so that
/// only identity/version-compatible clients connect to a given daemon.
/// <para>
/// These files are source-shared (linked) into both the thin client and the language server, so
