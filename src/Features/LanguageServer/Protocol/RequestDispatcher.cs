...
namespace Microsoft.CodeAnalysis.LanguageServer
{
    ...
    internal class RequestDispatcher : ILspService
    {
        ...
        private static ImmutableDictionary<RequestHandlerMetadata, Lazy<IRequestHandler>> CreateMethodToHandlerMap(LspServices lspServices)
        {
            ...
            var requestHandlers = lspServices.LazyLspServices.Where(kvp => IsTypeRequestHandler(kvp.Key));

            foreach (var requestHandlerService in requestHandlers)
            {
                var handlerType = requestHandlerService.Key;
                var lazyHandlerInstance = requestHandlerService.Value;
                ...
                {
                    var handlerInstance = lazyHandlerInstance.Value;
                    return (IRequestHandler)handlerInstance;
                }));
            }
