...
internal sealed class LspWorkspaceManager : IDocumentChangeTracker, ILspService
{
    ...
    /// <para/> Access to this is guaranteed to be serial by the <see cref="RequestExecutionQueue{RequestContextType}"/>
    /// </summary>
    private ImmutableDictionary<Uri, (SourceText Text, LanguageInformation? Language)> _trackedDocuments = ImmutableDictionary<Uri, (SourceText, LanguageInformation?)>.Empty;
    ...
    /// </summary>
    public async ValueTask StartTrackingAsync(Uri uri, SourceText documentText, LanguageInformation? languageInformation, CancellationToken cancellationToken)
    {
        ...
        _trackedDocuments = _trackedDocuments.Add(uri, (documentText, languageInformation));
    ...
    public ImmutableDictionary<Uri, SourceText> GetTrackedLspText() => _trackedDocuments.ToImmutableDictionary(kvp => kvp.Key, kvp => kvp.Value.Text);
    ...
    public async Task<(Workspace?, Solution?, Document?)> GetLspDocumentInfoAsync(TextDocumentIdentifier textDocumentIdentifier, CancellationToken cancellationToken)
    {
        ...
        {
            var miscDocument = _lspMiscellaneousFilesWorkspace?.AddMiscellaneousDocument(uri, trackedDocument.Text, trackedDocument.Language, _logger);
