...
namespace Microsoft.CodeAnalysis.LanguageServer.Handler.Implementation
{
    ...
    internal class CodeActionHandler : CodeActionsHandlerBase<LSP.CodeActionParams, object>
    {
        [ImportingConstructor]
        public CodeActionHandler(ICodeFixService codeFixService, ICodeRefactoringService codeRefactoringService) : base(codeFixService, codeRefactoringService)
        {
        }

        public override async Task<object> HandleRequestAsync(Solution solution, LSP.CodeActionParams request,
            LSP.ClientCapabilities clientCapabilities, CancellationToken cancellationToken)
        {
            ...
            foreach (var codeAction in codeActions)
            {
                ...
                {
                    Title = codeAction.Title,
                    CommandIdentifier = $"{RemoteCommandNamePrefix}.{LanguageServerProtocol.ProviderName}",
