...
internal class TestHistoryManager
{
    ...
    /// </summary>
    public static async Task<ImmutableDictionary<string, TimeSpan>> GetTestHistoryPerTypeAsync(/*int buildDefinitionId, string stageName, string branchName*/CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < totalTests; i += MaxTestsReturnedPerRequest)
        {
            ...
            foreach (var testResult in testResults)
            {
                ...
                if (!testInfos.TryAdd(testResult.AutomatedTestName, TimeSpan.FromMilliseconds(testResult.DurationInMs)))
                {
                    ...
                    Logger.Log($"Found tests with duplicate fully qualified name {testResult.TestCaseTitle}.");
                }
            }
        }
        ...
        // Group runtimes by fully qualified type name.
        var typeGroups = testInfos.GroupBy(kvp => GetTypeNameFromTestName(kvp.Key))
            .ToDictionary(group => group.Key, group => TimeSpan.FromMilliseconds(group.Select(g => g.Value).Sum(t => t.TotalMilliseconds)));

        var totalTestRuntime = TimeSpan.FromMilliseconds(typeGroups.Values.Sum(t => t.TotalMilliseconds));
        ConsoleUtil.WriteLine($"Retrieved {typeGroups.Keys.Count} types containing {testInfos.Count} total methods from AzureDevops in {timer.Elapsed}.  Total runtime of all tests is {totalTestRuntime}");
        return typeGroups.ToImmutableDictionary();
    }

    private static string GetTypeNameFromTestName(string fullyQualifiedTestName)
    {
        ...
        var lastPeriod = beforeMethodArgs.LastIndexOf(".");
        return beforeMethodArgs[..lastPeriod];
    }
