...
namespace RunTests
{
    internal record struct WorkItemInfo(ImmutableSortedDictionary<AssemblyInfo, ImmutableArray<MethodTestFilter>> Filters, int PartitionIndex)
    {
    ...
    {
        public TimeSpan GetExecutionTime() => Test.ExecutionTime;

        public string GetFilterString() => Test.FullyQualifiedName;
    }
    ...
    internal sealed class AssemblyScheduler
    {
        ...
        public async Task<ImmutableArray<WorkItemInfo>> ScheduleAsync(ImmutableArray<AssemblyInfo> assemblies, CancellationToken cancellationToken)
        {
            ...
            static ImmutableArray<WorkItemInfo> CreateWorkItemsForFullAssemblies(ImmutableArray<AssemblyInfo> assemblies)
            {
                ...
                {
                    var currentWorkItem = ImmutableSortedDictionary<AssemblyInfo, ImmutableArray<MethodTestFilter>>.Empty.Add(assembly, ImmutableArray<MethodTestFilter>.Empty);
        ...
        private static ImmutableArray<WorkItemInfo> BuildWorkItems(ImmutableSortedDictionary<AssemblyInfo, ImmutableArray<TypeInfo>> typeInfos, TimeSpan executionTimeLimit)
        {
            ...
            var currentFilters = new SortedDictionary<AssemblyInfo, List<MethodTestFilter>>();
            ...
            void AddFilter(AssemblyInfo assembly, MethodTestFilter filter)
            {
                ...
                {
                    var filterList = new List<MethodTestFilter>
                    {
                        filter
                    };
        ...
        private static void LogWorkItems(ImmutableArray<WorkItemInfo> workItems)
        {
            ...
            foreach (var workItem in workItems)
            {
                ...
                foreach (var assembly in workItem.Filters)
                {
                    ...
                    {
                        Logger.Log($"        - {testFilters.Length} tests: {string.Join(",", testFilters.Select(t => t.Test.FullyQualifiedName))}");
                    }
                }
            }
        }
