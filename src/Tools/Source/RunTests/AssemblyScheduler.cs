    ...
    internal sealed class AssemblyScheduler
    {
        ...
        private static ImmutableArray<TypeInfo> GetTypeInfoList(MetadataReader reader)
        {
            ...
            foreach (var handle in reader.TypeDefinitions)
            {
                ...
                var methodList = new List<TestMethodInfo>();
        ...
        private static void GetMethods(MetadataReader reader, TypeDefinition type, List<TestMethodInfo> methodList, string originalFullyQualifiedTypeName)
        {
            ...
            else if (!baseTypeHandle.IsNil)
            {
                ...
                methodList.Add(new TestMethodInfo(methodName, $"{originalFullyQualifiedTypeName}.{methodName}", TimeSpan.Zero));
            }
        }
        ...
        private static TestMethodInfo GetMethodInfo(MetadataReader reader, MethodDefinition method, string fullyQualifiedTypeName)
        {
            var methodName = reader.GetString(method.Name);
            return new TestMethodInfo(methodName, $"{fullyQualifiedTypeName}.{methodName}", TimeSpan.Zero);
        }
